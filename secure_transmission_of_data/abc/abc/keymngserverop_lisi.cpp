...
int KeyMngServerOp_LiSi::MngServer_Agree(MngServer_Info *svrInfo, MsgKey_Req *msgkeyReq, unsigned char **outData, int *datalen)
{
	...
	//写数据库
	LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"00000000000000000000000000");
	ret =  KeyMngDBOp::KeyMngsvr_DBOp_WriteSecKey(handle, &nodeSHMInfo);
	LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"1111111111111111111111111111");
	if (ret != 0)
	{
		LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"func KeyMngsvr_DBOp_WriteSecKey() err:%d \n", ret);
		goto End;
	}
	LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"222222222222222222222222");
	 
End:
	
	LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"33333333333333333333333");
	if  (ret == 0)
	{
		LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"444444444444444444444");
		IC_DBApi_Commit(handle); //提交事物
	}
	else
	{
		LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"444444444444444444-2");
		IC_DBApi_Rollback(handle); //回退
	}
	LogHelper::KeyMng_Log(__FILE__, __LINE__,LogHelper::KeyMngLevel[1], ret,"5555555555555555555555");
